...
function HomeClient({email}:{email?:string}) {
  ...
  },
  {
    ...
    desc: "Control exactly what the AI knows and answers."
  },
  {
    title: "Always Online",
    desc: "Customers get instant support 24/7."
  }
]
    ...
    <div className='min-h-screen bg-gradient-to-br from-white to-zinc-50 text-zinc-900 overflow-x-hidden'>
      ...
      transition={{duration:0.5}}
        className="fixed top-0 left-0 w-full z-50 bg-white/70 backdrop-blur-xl border-b border-zinc-200">
          ...
          <div className="text-lg font-semibold tracking-tight">Support <span className='text-zinc-400'>AI</span>
           </div>
           {email?<div className='relative' ref={popupRef}>
            <button className='w-10 h-10 rounded-full 
                      ...
                      onClick={()=>setOpen(!open)}>{firstLetter}</button>
                    <AnimatePresence>
                      {open && (
                        <motion.div
                        ...
                        >
                          ...
                          <button className='block px-4 py-3 text-sm text-red-600 
                          hover:bg-zinc-100' onClick={handleLogout}>Logout</button>
                        </motion.div>)}
                    </AnimatePresence>
           </div>:<button   
        ...
        <div className='max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-center'>
          <motion.div
            ...
            <p className='mt-6 text-lg text-zinc-600 max-w-xl'>
              AI Chatbot embedded to your Website - 
              Replace delays with intelligence, confusion with clarity, and support with a system that evolves alongside users.
            </p>
            <div className='mt-10 flex gap-4'>
              {email ? (
                ...
                hover:bg-zinc-800 transition 
                disabled:opacity-60' onClick={()=>navigate.push("/dashboard")}>Go to Dashboard</button>) : (
                <button 
          ...
          <div className='space-y-3'>
            <div className='bg-black text-white rounded-lg px-4 py-2 text-sm ml-auto w-fit'>
              Do you offer cash on delivery?</div>
      ...
      <section id='feature' className='bg-zinc-50 py-28 px-6 border-t border-zinc-200'>
        <div className='max-w-6xl max-auto'>
          ...
          className='text-3xl font-semibold text-center'>
            Why do Businesses Choose SupportAI 
          </motion.h2>
